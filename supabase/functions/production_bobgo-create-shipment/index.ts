...
serve(async (req) => {
  ...
  try {
    ...
    // Get BobGo API configuration (PRODUCTION versions)
    const BOBGO_API_KEY = Deno.env.get("PRODUCTION_BOBGO_API_KEY");
    ...
    function resolveBaseUrl() {
      const env = (Deno.env.get("PRODUCTION_BOBGO_BASE_URL") || "").trim().replace(/\/+$/, "");
