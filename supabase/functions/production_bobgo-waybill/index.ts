...
serve(async (req) => {
  ...
  try {
    ...
    // For production version, always use PRODUCTION_ prefixed vars
    const apiKey = (Deno.env.get("PRODUCTION_BOBGO_API_KEY") || "").trim();
    const baseUrlEnv = (Deno.env.get("PRODUCTION_BOBGO_BASE_URL") || "").trim();
    ...
    console.log("[get-waybill] Config:", { baseUrl, hasApiKey });

    if (!hasApiKey) {
      return new Response(
        JSON.stringify({ success: false, error: "MISSING_BOBGO_API_KEY", missing: "PRODUCTION_BOBGO_API_KEY" }),
    ...
    const waybillUrl = `${baseUrl}/shipments/waybill?tracking_references=${encodeURIComponent(trackingRefsParam)}`;
